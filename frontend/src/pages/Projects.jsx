...
const EMPTY = { title: '', client_name: '', project_type: 'events', event_date: '', revenue: '', cost: '', status: 'pending', notes: '', period_month: '', period_year: '' };

function ProjectModal({ project, onSave, onClose }) {
  const { user } = useAuth();
  const [form, setForm] = useState(project || { ...EMPTY, period_month: new Date().getMonth() + 1, period_year: new Date().getFullYear() });
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-end">
              <div className="text-sm">GP: <span className={`font-semibold ${gp >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatCurrency(gp)}</span></div>
            </div>
            <div>
              <label className="label">Period Month</label>
              <select className="input" value={form.period_month} onChange={e => setForm(f => ({ ...f, period_month: parseInt(e.target.value) }))}>
                {Array.from({ length: 12 }, (_, i) => <option key={i + 1} value={i + 1}>{monthName(i + 1)}</option>)}
              </select>
            </div>
            <div>
              <label className="label">Period Year</label>
              <select className="input" value={form.period_year} onChange={e => setForm(f => ({ ...f, period_year: parseInt(e.target.value) }))}>
                {[2024, 2025, 2026].map(y => <option key={y}>{y}</option>)}
              </select>
            </div>
            {user.role === 'bdm' && users.length > 0 && (
              <div className="col-span-2">
...
export default function Projects() {
  ...
  const totalGP = projects.filter(p => ['confirmed', 'completed'].includes(p.status)).reduce((s, p) => s + parseFloat(p.gp || 0), 0);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <div className="ml-auto text-sm font-semibold text-gray-700">Total GP: <span className="text-green-600">{formatCurrency(totalGP)}</span></div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-500 uppercase border-b">
                ...
                <th className="text-right py-2 px-3">GP</th>
                <th className="text-left py-2 px-3">Status</th>
                {user.role === 'bdm' && <th className="text-left py-2 px-3">Assigned</th>}
            ...
            <tbody className="divide-y divide-gray-100">
              {projects.map(p => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-2.5 px-3 text-gray-600">{p.client_name}</td>
                  <td className="py-2.5 px-3 text-gray-500 capitalize">{p.project_type.replace('_', ' ')}</td>
                  <td className="py-2.5 px-3 text-gray-500">{p.event_date ? new Date(p.event_date).toLocaleDateString('en-SG') : '—'}</td>
                  ...
                  <td className="py-2.5 px-3 text-right font-semibold text-green-700">{formatCurrency(p.gp)}</td>
                  <td className="py-2.5 px-3"><span className={`badge ${STATUS_COLORS[p.status]}`}>{p.status}</span></td>
                  {user.role === 'bdm' && <td className="py-2.5 px-3 text-gray-500 text-xs">{p.assigned_name}</td>}
